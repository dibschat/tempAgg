...
from dataset_anticipation import SequenceDataset
from os.path import join
import torch
from torch.utils.data import DataLoader
import torch.nn.functional as F
from utils import ValueMeter, topk_accuracy, topk_accuracy_save_validation_pred, topk_recall, MeanTopKRecallMeter
...
parser.add_argument('--mode', type=str, default='train', choices=['train', 'validate', 'train_val', 'test', 'validate_json'],
...
parser.add_argument('--path_to_models', type=str, default=COMP_PATH + '/models_anticipation/',
                    help="Path to the directory where to save all models")

parser.add_argument('--json_directory', type=str, default=COMP_PATH + '/models_anticipation/',
                    help='Directory in which to save the generated jsons.')
parser.add_argument('--task', type=str, default='action_anticipation',
...
parser.add_argument('--weight_rgb', type=float, default=0.4, help='')
parser.add_argument('--weight_flow', type=float, default=0.1, help='')
parser.add_argument('--weight_obj', type=float, default=0.25, help='')
parser.add_argument('--weight_roi', type=float, default=0.25, help='')
...
parser.add_argument('--add_noun_loss', type=bool, default=True, help='Whether to train with verb loss or not')
...
def make_model_name(arg_save):
    save_name = "anti_mod_{}_span_{}_s1_{}_s2_{}_s3_{}_recent_{}_r1_{}_r2_{}_r3_{}_r4_{}_bs_{}_drop_{}_lr_{}_dimLa_{}_" \
...
def get_scores(model, loader, challenge=False, include_discarded=False):
    ...
    with torch.set_grad_enabled(False):
        for batch in tqdm(loader, 'Evaluating...', len(loader)):
            ...
            if args.add_verb_loss and args.add_noun_loss:
...
def get_scores_late_fusion(models, loaders, challenge=False, include_discarded=False):
    verb_scores = []
    noun_scores = []
    action_scores = []
    outputs = []
    for model, loader in zip(models, loaders):
        outputs = get_scores(model, loader, challenge, include_discarded)
...
def train_validation(model, loaders, optimizer, epochs, start_epoch, start_best_perf, schedule_on):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        ensemble_accuracy_meter = {'training': MeanTopKRecallMeter(args.num_class), 'validation': MeanTopKRecallMeter(args.num_class)}
...
def main():
    ...
    if args.mode == 'train':
        ...
        if args.modality == 'late_fusion':
            ...
            verb_scores, noun_scores, action_scores, verb_labels, noun_labels, action_labels, ids = get_scores_late_fusion(model, loaders, include_discarded=args.ek100)
        else:
        ...
        if not args.ek100:
            ...
            noun_accuracies = topk_accuracy(noun_scores, noun_labels, (1,))[0]
            action_accuracies = topk_accuracy_save_validation_pred(action_scores, action_labels, (1,), no_classes = args.num_class)[0]

            many_shot_verbs, many_shot_nouns, many_shot_actions = get_many_shot()

            verb_recalls = topk_recall(verb_scores, verb_labels, k=1, classes=many_shot_verbs)
            noun_recalls = topk_recall(noun_scores, noun_labels, k=1, classes=many_shot_nouns)
            action_recalls = topk_recall(action_scores, action_labels, k=1, classes=many_shot_actions)

            print("Verb Accuracy: {:.2f}%".format(verb_accuracies * 100))
            print("Noun Accuracy: {:.2f}%".format(noun_accuracies * 100))
            print("Action Accuracy: {:.2f}%".format(action_accuracies * 100))
            print("Verb Recall: {:.2f}%".format(verb_recalls * 100))
            print("Noun Recall: {:.2f}%".format(noun_recalls * 100))
            print("Action Recall: {:.2f}%".format(action_recalls * 100))

        else:
            many_shot_verbs, many_shot_nouns, many_shot_actions = get_many_shot()
            verb_accuracies = topk_accuracy(verb_scores, verb_labels, (1,))[0]
            noun_accuracies = topk_accuracy(noun_scores, noun_labels, (1,))[0]
            action_accuracies = topk_accuracy_save_validation_pred(action_scores, action_labels, (1,), 
                                                modality = args.modality, no_classes = args.num_class)[0]
            verb_recalls = topk_recall(verb_scores, verb_labels, k=1)
            noun_recalls = topk_recall(noun_scores, noun_labels, k=1)
            action_recalls = topk_recall(action_scores, action_labels, k=1)
            
            overall_verb_recalls = topk_recall(verb_scores, verb_labels, k=5)
            overall_noun_recalls = topk_recall(noun_scores, noun_labels, k=5)
            overall_action_recalls = topk_recall(action_scores, action_labels, k=5)
            ...
            tail_verb_recalls = topk_recall(verb_scores[tail_verbs_bool_idx], verb_labels[tail_verbs_bool_idx], k=5)
            tail_noun_recalls = topk_recall(noun_scores[tail_nouns_bool_idx], noun_labels[tail_nouns_bool_idx], k=5)
            tail_action_recalls = topk_recall(action_scores[tail_actions_bool_idx], action_labels[tail_actions_bool_idx], k=5)


            unseen_verb_recalls = topk_recall(verb_scores[unseen_bool_idx], verb_labels[unseen_bool_idx], k=5)
            unseen_noun_recalls = topk_recall(noun_scores[unseen_bool_idx], noun_labels[unseen_bool_idx], k=5)
            unseen_action_recalls = topk_recall(action_scores[unseen_bool_idx], action_labels[unseen_bool_idx], k=5)

            print(f'Overall Mean Top-5 Recall (Verb) = {overall_verb_recalls*100:.2f}')
            print(f'Overall Mean Top-5 Recall (Noun) = {overall_noun_recalls*100:.2f}')
            print(f'Overall Mean Top-5 Recall (Action) = {overall_action_recalls*100:.2f}')
            print(f'Unseen Mean Top-5 Recall (Verb) = {unseen_verb_recalls*100:.2f}')
            print(f'Unseen Mean Top-5 Recall (Noun) = {unseen_noun_recalls*100:.2f}')
            print(f'Unseen Mean Top-5 Recall (Action) = {unseen_action_recalls*100:.2f}')
            print(f'Tail Mean Top-5 Recall (Verb) = {tail_verb_recalls*100:.2f}')
            print(f'Tail Mean Top-5 Recall (Noun) = {tail_noun_recalls*100:.2f}')
            print(f'Tail Mean Top-5 Recall (Action) = {tail_action_recalls*100:.2f}')
